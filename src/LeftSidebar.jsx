            ...
            <ul className="p-4 flex xl:flex-col items-stetch gap-2 text-lg xl:fixed overflow-x-auto xl:overflow-y-auto xl:h-screen xl:w-[100cqw]">
                {subjects.map(subject => (
                    <li key={subject.name} className="flex">
                        <Link to={`/Cruxian/problems/${subject.name}/${subject.topics[0].name}`} className={`truncate rounded p-2 cursor-pointer flex-1 ${subject.name === subjectName ? "" : "hover:"}text-white transition duration-200`}>{subject.name}</Link>
                    </li>
                ))}
            </ul>
        </header>
